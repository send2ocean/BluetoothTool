...
/**
 * @author XiongChuanLiang<br />(xcl_168@aliyun.com) -- 修改
 * @ClassName LineChart
 * @Description 线图基类
 */
...
//                                canvas, pLine.getLinePaint());
                        ...
                        if (count > 1) {
                            DrawHelper.getInstance().drawLine(bd.getLineStyle(),
                                    lineStartX, lineStartY, initX + i * ConantPalmer.distance1, lineStopY,
                                    canvas, pLine.getLinePaint());
                        }
                    }
...
//                        lineStopX = add(lineStopX, radius);
                        ...
                        PlotDotRender.getInstance().renderDot(canvas, pDot,
                                initX + i * ConantPalmer.distance1, lineStopY,
                                pLine.getDotPaint()); //标识图形

                        savePointRecord(dataID, i, initX + i * ConantPalmer.distance1 - radius + mMoveX, lineStopY + mMoveY,
                                initX + i * ConantPalmer.distance1 - radius + mMoveX,
                                lineStopY - radius + mMoveY,
                                initX + i * ConantPalmer.distance1 + radius + mMoveX,
                                lineStopY + radius + mMoveY);
...
//                                itemAngle, bd.getLineColor());
                        ...
                        bd.getLabelOptions().drawLabel(canvas, pLine.getDotLabelPaint(),
                                getFormatterItemLabel(bv), initX + i * ConantPalmer.distance1 + 2, lineStopY,
                                itemAngle, bd.getLineColor());
                    }
...
//                lineStartX = lineStopX;
                lineStartX = initX + i * ConantPalmer.distance1;
                lineStartY = lineStopY;
