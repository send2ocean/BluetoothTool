...
public class LnChart extends AxesChart {
    ...
    protected void drawClipCategoryAxisGridlines(Canvas canvas) {
        ...
        System.out.println("LnChart获取的点数 : " + tickCount);
        // 绘制
        for (int i = 0; i < tickCount; i++) {
            switch (pos) {
                ...
                case HORIZONTAL_CENTER:
                    ...
                    // 绘制竖向网格线
                    drawVerticalGridLines(canvas, plotArea.getTop(),
                            plotArea.getBottom(), i, tickCount, i1 + i * ConantPalmer.distance1, i1 + i * ConantPalmer.distance1);
                    Log.e("====", i1 + i * ConantPalmer.distance1 + "");
                    if (!categoryAxis.isShowAxisLabels())
                        continue;
...
//				 mLstCateTick.add(new PlotAxisTick( currentX, axisY, dataSet.get(i),labelX,labelY,showTicks));
                    mLstCateTick.add(new PlotAxisTick(i1 + i * ConantPalmer.distance1, axisY, dataSet.get(i), i1 + i * ConantPalmer.distance1, labelY, showTicks));
                    break;
            } // switch end
            j++;
        } // end for
