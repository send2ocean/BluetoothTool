...
//		plotArea.setTop(add(this.getTop() + borderWidth/2 , mPaddingTop));
        ...
        plotArea.setBottom(sub(this.getBottom() - borderWidth / 2, mPaddingBottom) - 20);
        plotArea.setLeft(add(this.getLeft() + borderWidth / 2, mPaddingLeft) + ScreenUtil.dip2px(App.mContext, 11));
        //设置的是两个数字之间的间隔 修改出来啊
        ...
        if (dataSize > 0) {
            ...
            int i1 = ScreenUtil.dip2px(App.mContext, 30);
            plotArea.setRight(size * (150 + i1));
        } else {
            plotArea.setRight(1000 * 10);
        }
        plotArea.setTop(add(this.getTop() + borderWidth / 2, mPaddingTop));
//        int borderWidth = getBorderWidth();
//        if(null == plotArea) return;
...
//        plotArea.setTop(add(this.getTop() + borderWidth/2 , mPaddingTop));
    ...
    public DyLine getDyLine() {
        ...
        mDyLine.setDyLineStyle(XEnum.DyLineStyle.Vertical);
        mDyLine.setLineDrawStyle(XEnum.LineStyle.DASH);
        return mDyLine;
    }
