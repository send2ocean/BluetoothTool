...
public class AxesChart extends EventChart {
    ...
    protected void drawHorizontalGridbias(Canvas canvas, float plotLeft, float plotRight,
                                          int tickID, int tickCount,
                                          float YSteps, float startY, float currentY) {
        if (tickID < 0) return; //tickID <= 0
        ...
        if (tickID >= 0 && tickID <= tickCount) {
            plotGrid.setPrimaryTickLine(dataAxis.isPrimaryTick(tickID));
            plotGrid.renderGridLinesHorizontal(canvas, plotLeft, startY, plotRight, currentY);
        }
    }
    ...
    protected boolean drawClipVerticalPlot(Canvas canvas) {
        ...
        canvas.clipRect(plotArea.getLeft() - 5 - 20,
                plotArea.getTop() - getClipExt().getExtTop() - 100,
