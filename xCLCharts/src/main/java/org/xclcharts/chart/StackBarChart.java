...
//				    flatBar.renderBar(currentX ,topY,rightX,bottomY,canvas);
					 float v = (topY - bottomY) / 7;

					 //固定住stackBarChartView的位置
					 if(i==0){
						 flatBar.renderBar(currentX, 70f, rightX, 110f, canvas, 1);//固定位置
//						 flatBar.renderBar(currentX ,topY,rightX,bottomY,canvas);
					 }
					 if(i==1){
						 flatBar.renderBar(currentX, 70f - v, rightX, 110f + v, canvas, 1);//固定位置
//
//						 flatBar.renderBar(currentX ,topY-v,rightX,bottomY+v,canvas);
					 }
				    //保存位置
					 saveBarRectFRecord(i, r, currentX + mMoveX, 70f, rightX + mMoveX, bottomY + mMoveY);//固定位置
			...
			float barWidth = getVBarWidth(XSteps); // mul(XSteps,0.5f);	
			 ...
			 {				 				 		
					 ...
					 for(int i=0; i < sourceSize;i++) //各自所占的高度
					 {
						...
						flatBar.renderBar(leftX, topY, rightX, currentY, canvas, 1);
						//保存位置
