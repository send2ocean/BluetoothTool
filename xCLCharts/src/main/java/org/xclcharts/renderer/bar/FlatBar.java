...
public class FlatBar extends Bar{
	 ...
	 * @param canvas	画布
	 */
	public boolean renderBar(float left,float top,float right ,float bottom,Canvas canvas, int i)
	{
		...
		if(XEnum.BarStyle.OUTLINE == style)
		{
			...
			drawPathBar(left,top,right,bottom,canvas, i);
	...
	private void drawPathBar(float left,float top,float right,float bottom,Canvas canvas, int i)
	{
		mPath.moveTo(left, bottom);
		...
		if (i == 0){
			LinearGradient linearGradient =
					new LinearGradient(left, top, right, bottom, Color.parseColor("#00ec00"), Color.parseColor("#00A800"), Shader.TileMode.CLAMP);
