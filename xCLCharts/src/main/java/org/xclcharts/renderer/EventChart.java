...
public class EventChart extends XChart {
	...
	public boolean isPlotClickArea(float x, float y) {
		...
		System.out.println ("触摸点的位置 : " + x + " : " + y);
		System.out.println ("left : " + getPlotArea().getLeft());
		System.out.println ("right : " + getPlotArea().getRight());
		System.out.println ("top ; " + getPlotArea().getTop());
		System.out.println ("bottom : " + getPlotArea().getBottom());
		float top = getPlotArea().getTop() - 200;
		float right = getPlotArea().getRight() + 5;
		float left = getPlotArea().getLeft() - 5;
		float bottom = getPlotArea().getBottom() + 200;
		if (!getListenItemClickStatus()) return false;
		if (Float.compare(x, left)  == -1) return false;
		if (Float.compare(x, right) == 1) return false;
		if (Float.compare(y, top) == -1) return false;
		if (Float.compare(y, bottom) == 1) return false;
		System.out.println ("isPlotClickArea通过了");
